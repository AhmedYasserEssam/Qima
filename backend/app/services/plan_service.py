...
def _simple_generated_plan(
    ...
    for day_number in range(1, plan_days + 1):
        ...
        for meal_index, calories in enumerate(meal_calories):
            meal = _simple_meal(payload, meal_index, calories)
            meals.append(meal)
    ...
    return (
        {
            ...
            "daily_summary": {
                ...
                "strategy": "Fast local plan with common meals, simple portions, and no model call.",
...
def _meal_candidates_from_generated_plan(
    ...
    for day in generated_plan.get("plan") or []:
...
def _generated_plan_quality_warnings(
    ...
    warnings: list[str] = []
    plan = generated_plan.get("plan") or []
    expected_days = (
